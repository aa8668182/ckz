...
import com.qianmi.marketing.groupon.service.MarketingGrouponOrderHandlerService;
...
@Slf4j
@Component
public class GrouponQuartzJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (marketingGrouponInstVo.getStatus() == Constants.MarketingGroupon.ORDER_MARKETING_GROUPON_COMPLETE || marketingGrouponInstVo.getStatus() == Constants.MarketingGroupon.ORDER_MARKETING_GROUPON_DEL){
            ...
            Date endTime =  marketingGrouponInstService.getMarketingGrouponInstEndTime(marketingGrouponInstVo.getStartTime(),marketingGrouponInstVo.getMarketing().getEndTime());
            ...
            if (endTime.getTime() < DateUtil.now().getTime()  && marketingGrouponInstVo.getStatus() == Constants.MarketingGroupon.ORDER_MARKETING_GROUPON_STATUS){
                MarketingGrouponOrderHandlerService marketingGrouponOrderHandlerService =  (MarketingGrouponOrderHandlerService)ApplicationContextHelper.getBean("marketingGrouponOrderHandlerService");
                marketingGrouponOrderHandlerService.handlerGroupOrderByGroupNo(grouponNo,"超过拼团结束时长24小时，未成团");
                marketingGrouponInstService.sendWechatTmpMsgGrouponFailed(grouponNo);//发送拼团失败-微信公众号模板消息
                //移除当前团编号的定时任务,并更新数据库状态信息
                updateTaskJobAndMoveFromScheduler(grouponNo,jobId);
...
//            quartzManagerService.modifyJobTime(grouponNo,Constants.MarketingGroupon.JOB_GROUP_NAME,DateUtil.dateToString(endTime,"ss mm HH dd")+DateUtil.dateToString(endTime," MM")+" ? *");
            quartzManagerService.modifyJobTime(grouponNo,Constants.MarketingGroupon.JOB_GROUP_NAME,DateUtil.dateToString(endTime,"ss mm HH")+" * * ? *");
    ...
    private void updateTaskJobAndMoveFromScheduler(String grouponNo,Integer jobId){
        //移除当前团编号的定时任务
        QuartzManagerService quartzManagerService  =  (QuartzManagerService)ApplicationContextHelper.getBean("quartzManagerService");
        quartzManagerService.removeJob(grouponNo,Constants.MarketingGroupon.JOB_GROUP_NAME,grouponNo,Constants.MarketingGroupon.JOB_GROUP_NAME);
